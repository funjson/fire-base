...
import dev.infinityknowledge.controlplane.api.governance.ManagementViews;
...
public final class KnowledgeManagementService {
    ...
    public ManagementViews.Page<ManagementViews.Document> documents(
        ...
        List<ManagementViews.Document> items = page.items().stream()
                .map(KnowledgeManagementService::document)
                .toList();
    ...
    /** 读取独立文档详情页摘要；租户条件由存储端口强制执行。 */
    public ManagementViews.Document document(
            PrincipalContext principal,
            UUID documentId
    ) {
        requireAdmin(principal);
        return store.document(principal.tenantId(), documentId)
                .map(KnowledgeManagementService::document)
    ...
    private static ManagementViews.Document document(
            KnowledgeAdministrationStore.Document value
    ) {
        return new ManagementViews.Document(
                value.id(),
                value.spaceId(),
                value.title(),
                value.sourceType(),
